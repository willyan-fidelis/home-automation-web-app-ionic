var MqttClient = function(args) { // eslint-disable-line no-unused-vars
    ...
    self.once   = function(event, func) {
        //throw new Error(format({code:11, text:"AMQJS0011E Invalid state {0}."}, ["for test reasons!"]));
    ...
    if(self.broker.path !== ""){
      self.client = new Paho.MQTT.Client(self.broker.host, self.broker.port, self.broker.path, self.broker.clientId);
    }
    else{
